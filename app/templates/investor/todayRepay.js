...
angular.module('myApp.todayRepay', ['ngRoute'])

    .config(['$routeProvider', function ($routeProvider) {
        $routeProvider.when('/investor/todayRepay', {
            templateUrl: 'templates/investor/todayRepay.html',
            controller: 'todayRepayCtrl'
        });
    }])

    .controller('todayRepayCtrl', function ($scope, $http, $mdDialog, riskService,investorCashCtrlService,$filter) {
  	...
  	if(x.status==1){
        self.location = "/manageSystem/#/money/todayRepayExamine/"+x.id;
    }
        ...
        $scope.selectPage = function (page) {
            ...
            investorCashCtrlService.selectPage("/plan/repay/list",data).then(function () {
...
$scope.cashData1={
        ...
        $scope.gotoExport=function(){
           ...
           $scope.cashData1['planName']=$scope.planName;
            ...
            window.open(HOST_URL + "/invest/repay/list/export?startDate="+data1.startDate+"&endDate="+data1.endDate+"&planName="+data1.planName+"&keyword="+data1.keyword+"&status="+data1.status+"&channel="+data1.channel+"&token=" + token );
