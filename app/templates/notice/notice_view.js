...
angular.module('myApp.notice_view', ['ngRoute'])

    .config(['$routeProvider', function ($routeProvider) {
        $routeProvider.when('/notice/notice_view', {
            templateUrl: 'templates/notice/notice_view.html',
            controller: 'NoticeViewCtrl'
        });
    }])

    .controller('NoticeViewCtrl', function ($scope,$location,$mdDialog, $http,IndexService,riskService,investorCashCtrlService,$routeParams,bannerListService) {
        $scope.news = {};
        $scope.userAccount = {};
		$scope.title="";
        // 检测登录
		...
		$scope.gotoedit=function(id){
			$location.url("notice/notice_edit?id="+id);
		};
		// 查询条件
        $scope.cashData = {
            page: 1,
		 ...
		 $scope.selectPage = function (page) {
			...
			// $scope.cashData['title']=$scope.title
             ...
             investorCashCtrlService.selectPage("/notice/list", data).then(function () {
...
$scope.showConfirm=function(notice){
    $mdDialog.show(
        ...
        $scope.deleteNews=function(notice){

            $http.get(HOST_URL + "/notice/"+notice.id+"/delete",
                {
                    params:data
                },
                {
