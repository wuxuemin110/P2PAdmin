...
angular.module('myApp.activity_list', ['ngRoute'])

    .config(['$routeProvider', function ($routeProvider) {
        $routeProvider.when('/activity/activity_list', {
            templateUrl: 'templates/activity/activity_list.html',
            controller: 'activity_listCtrl'
        });
    }])

    .controller('activity_listCtrl', function ($scope,$location,  IndexService,investorCashCtrlService,bannerListService,$mdDialog,$filter,activity_listCtrlService) {
        $scope.tradeRecord = {};
        $scope.userAccount = {};
        var type = "index";
        $scope.isActive = true; 
        ...
        $scope.showConfirm=function(id){
            $mdDialog.show(
                ...
                activity_listCtrlService.delNews(token,id);
            }, function () {
		...
		$scope.gotoEdit=function(id,type){
			if(type=="activity"){
				$location.url("/activity/activityEdit?id="+id);
			}
			else if(type=="activityMobile"){
				$location.url("/activity/activityMobileEdit?id="+id);
			}
...
//          console.log(data);
            investorCashCtrlService.selectPage("/activity/list", data).then(function () {
...
//        console.log(tmpObject);
    ...
    .factory('activity_listCtrlService', function ($http, $mdDialog,bannerListService) {
        var tradeRecords;
        return {
			...
			"token": token,
			 ...
			 return $http.get(HOST_URL + "/activity/" + id + "/delete",
			  {
				params: tokenData
			}, {
				headers: {
